...
public class EspecialidadController {
	...
	//Metodo handler para ver el detalle de la especialidad a travez del id.
		...
		//Metodo editar Paciente		
			@GetMapping("/formEspecialidad/{id}")
			...
			public String eliminarEspecialidad(@PathVariable(value="id") Long id, RedirectAttributes flash )
			{
				if(id > 0)
				{
					...
					//verificamos si la especialidad tiene planillas activas
					if(planillaService.ExisteEspecialidadEnPanillaCabecera(id) != null && !planillaService.ExisteEspecialidadEnPanillaCabecera(id).isEmpty())
					{
						flash.addFlashAttribute("error",
								"ERROR - La Especialidad posee Planillas Activas o Inactivas!. Verifique..");
						return "redirect:/especialidades/listarEspecialidad";
					}
