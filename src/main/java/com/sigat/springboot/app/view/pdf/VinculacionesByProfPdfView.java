...
@Component("vinculacionesByProfPdfView")
public class VinculacionesByProfPdfView extends AbstractPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		String nombreArchivo = "vinculacionPorProf_" + fecha + ".pdf";
		...
		List<Vinculacion> vinculacionesbyprof = (List<Vinculacion>) model.get("vinculacionesbyprof");
		...
		String titulo = "Listado de Vinculaciones por profesional";
		...
		// creamos tabla
		PdfPTable tablaVinculacionesByProf = new PdfPTable(3);
		// tablaVinculacionesByProf.setSpacingAfter(20);
		// Ancho de la primera columna
		tablaVinculacionesByProf.setWidthPercentage(100);
		tablaVinculacionesByProf.getDefaultCell().setBorder(0);
		// tablaVinculacionesByProf.setSpacingBefore(220);
		tablaVinculacionesByProf.setSpacingAfter(15);
		// Define los anchos absolutos de cada columna
		float[] columnWidths = { 240f, 240f, 650f };
		// Asigna los anchos a la tabla
		tablaVinculacionesByProf.setWidths(columnWidths);

		// Agregamos a la tabla los encabezados y su formato.
		tablaVinculacionesByProf.addCell(cellProfesional);
		tablaVinculacionesByProf.addCell(cellEspecialidadVinculada);
		tablaVinculacionesByProf.addCell(cellObservaciones);

		// Recorrer el List<> de turnos ocupados.
		for (int i = 0; i < vinculacionesbyprof.size(); i++) {

			// Agregar una fila a la tabla
			Vinculacion item = vinculacionesbyprof.get(i);
			...
			tablaVinculacionesByProf.addCell(celdaProfesional);
			tablaVinculacionesByProf.addCell(celdaEspecVinculada);
			tablaVinculacionesByProf.addCell(celdaObserv);
			
			// Si se alcanza el límite de filas por página o es el último elemento, añadir
			// la tabla y crear una nueva página
			if ((i + 1) % ROWS_PER_PAGE == 0 || (i + 1) == vinculacionesbyprof.size()) {
				document.add(img);
				document.add(tablaTitulo);
				document.add(tableFechaHora);
				document.add(tablaVinculacionesByProf);
				...
				tablaVinculacionesByProf.flushContent(); // Limpiar la tabla para la siguiente página

				if ((i + 1) < vinculacionesbyprof.size()) {
